...
export async function GET(req) {
    ...
    try {
        ...
        await connect();
        const user = await Hod.findById(decoded.id)
        if (!user) {
            return new Response(JSON.stringify({ message: "HoD Not found" }), { status: 404 })
        }
        return new Response(JSON.stringify({ message: "Success", user }), { status: 200 })
    } catch (error) {
        return new Response(JSON.stringify({ message: "Error getting the HoD", error: error.message }), { status: 500 });
    }
}